...
export const Admin: React.FC = () => {
  ...
  const [clientCharges, setClientCharges] = useState<any[]>([]);
  const [chargeForm, setChargeForm] = useState({ label: "", amount: "", currency: "usd" });
  ...
  useEffect(() => {
    ...
    if (activeView === "clients") {
      loadClientCharges(selectedBillingClientId || undefined);
    }
  }, [activeView, isAuth, selectedBillingClientId]);
  ...
  useEffect(() => {
    if (!clients.length || selectedBillingClientId) return;
    const first = clients[0];
    if (first?.id) {
      setSelectedBillingClientId(first.id);
      loadClientCharges(first.id);
    }
  ...
  const loadClientCharges = async (clientId?: string) => {
    const targetClientId = clientId || selectedBillingClientId;
    if (!targetClientId) {
      setClientCharges([]);
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.charges) {
        ...
        setChargeStatus(data.error || "Unable to load charges");
      }
    } catch (err: any) {
      setChargeStatus(err?.message || "Unable to load charges");
    } finally {
  ...
  const handleCreateCharge = async () => {
    if (!selectedBillingClientId) {
      setChargeStatus("Select a client to create a charge.");
      return;
    }
    if (!chargeForm.label.trim() || !chargeForm.amount) {
      setChargeStatus("Label and amount are required.");
      return;
    }
    const amountNumber = Number(chargeForm.amount);
    if (!Number.isFinite(amountNumber) || amountNumber <= 0) {
      setChargeStatus("Enter a valid amount.");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/admin/clients/${selectedBillingClientId}/charges`, {
        ...
        body: JSON.stringify({
          ...
          amount: amountNumber,
          currency: chargeForm.currency || "usd",
        }),
      });
      const data = await res.json();
      ...
      setChargeStatus(err?.message || "Unable to create charge");
    } finally {
  ...
  const handleUpdateChargeStatus = async (id: string, status: "pending" | "paid") => {
    ...
    try {
      ...
      setChargeStatus(err?.message || "Unable to update charge");
    }
  };
                ...
                <div className={`${panelClass} p-6 md:p-8 space-y-6`}>
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-white">Billing for this client</h3>
                      <p className="text-muted text-sm">Create simple charges and track their payment status.</p>
                    </div>
                    <div className="flex flex-col md:flex-row gap-2 w-full md:w-auto">
                      <select
                        className="w-full bg-background/80 border border-white/10 rounded-lg p-3 text-white text-sm"
                        value={selectedBillingClientId}
                        onChange={(e) => {
                          const clientId = e.target.value;
                          setSelectedBillingClientId(clientId);
                          if (clientId) {
                            loadClientCharges(clientId);
                          } else {
                            setClientCharges([]);
                          }
                        }}
                      ...
                      <Button variant="outline" onClick={() => loadClientCharges(selectedBillingClientId || undefined)} className="text-sm">
                  ...
                  {selectedBillingClientId ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <p className="text-sm text-muted">Create a new charge</p>
                        <input
                          className="w-full bg-background/80 border border-white/10 rounded-lg p-3 text-white"
                          placeholder="Label (e.g. Phase 1 invoice)"
                        ...
                        <Button onClick={handleCreateCharge} disabled={chargeSaving} className="text-sm">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <p className="text-sm text-muted">Recent charges</p>
                          {chargesLoading && (
                        ...
                        {chargesLoading ? (
                          <div className="text-sm text-muted">Loading charges...</div>
                          ...
                          <div className="space-y-3">
                            {clientCharges.map((charge) => {
                              ...
                              return (
                                <div key={charge.id} className="p-3 border border-white/10 rounded-lg bg-white/5 flex flex-col gap-2">
                                  <div className="flex items-center justify-between gap-3">
                                    ...
                                    <div className="text-right space-y-1">
                                      ...
                                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-[11px] border ${isPaid ? "border-green-400/40 text-green-200" : "border-amber-400/40 text-amber-200"}`}>
                                        {isPaid ? "Paid" : "Pending"}
                                      </span>
                                    </div>
                                  </div>
                                  {!isPaid && (
                                    <div className="flex items-center justify-between text-xs text-muted">
                                      <span>Client not paid yet.</span>
                                      <Button variant="outline" size="sm" onClick={() => handleUpdateChargeStatus(charge.id, "paid")}>
                                        Mark paid
                                      </Button>
                                    </div>
                                  )}
                          ...
                          <p className="text-sm text-muted">No charges yet for this client.</p>
                        )}
                      </div>
                    </div>
                  ) : (
                    <p className="text-sm text-muted">Select a client to manage billing.</p>
                  )}
                </div>
